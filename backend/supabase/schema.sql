...
-- The `app.bulk_rewrite` escape hatch exists because the home screen's
-- "Recent updates" tile sorts on this column, so updated_at has to mean
-- "a person changed this". Two operations rewrite rows without anyone
-- touching an item -- rename_room() rewriting items.room for a whole
-- room, and the FK cascade nulling created_by when an account is deleted
-- -- and both would otherwise float untouched items to the top of that
-- tile. A column-level check can't tell them apart from real edits (a
-- rename and a genuine move are both `update items set room`), so the
-- callers declare it instead. set_config's third argument makes the flag
-- transaction-local, so it's discarded at commit or rollback and can't
...
begin
  ...
  -- anyone moving anything, so don't let it read as recent activity.
...
begin
  ...
  -- person added. Those items are unchanged as far as the household is
  -- concerned, so they shouldn't resurface as recent activity.
  perform set_config('app.bulk_rewrite', 'on', true);
